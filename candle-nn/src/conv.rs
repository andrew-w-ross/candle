...
/// Configuration for [`Conv3d`]. Spatial parameters are ordered `[depth, height, width]`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Creates a 3D convolution layer, `kernel_size` is given as `[depth, height, width]`.
pub fn conv3d(
    in_channels: usize,
    out_channels: usize,
    kernel_size: [usize; 3],
